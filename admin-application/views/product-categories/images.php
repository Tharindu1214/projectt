...
<ul class="grids--onethird" id="<?php if ($canEdit) { ?>sortable<?php } ?>">
    ...
    <li id="<?php echo $row['afile_id']; ?>">
        <div class="logoWrap">
            <div class="logothumb"> <img src="<?php echo CommonHelper::generateUrl('Category', $imageFunction, array($row['afile_record_id'], $row['afile_lang_id'], "THUMB", $row['afile_screen']), CONF_WEBROOT_FRONT_URL); ?>"
                    ...
                    onclick="deleteImage(<?php echo $row['afile_id']; ?>, <?php echo $row['afile_record_id']; ?>, '<?php echo $imageType; ?>', <?php echo $row['afile_lang_id']; ?>, <?php echo $row['afile_screen']; ?> );" class="delete"><i class="ion-close-round"></i></a>
                <?php } ?>
            </div>
