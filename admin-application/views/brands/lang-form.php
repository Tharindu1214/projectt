		...
		<ul class="tabs_nav">
			...
			if ($brand_id > 0) {
				foreach($languages as $langId=>$langName){?>
					<li><a class="<?php echo ($brand_lang_id==$langId)?'active':''?>" href="javascript:void(0);" onclick="brandLangForm(<?php echo $brand_id ?>, <?php echo $langId;?>);"><?php echo labels::getLabel("LBL_".$langName,$adminLangId);?></a></li>
				<?php }
				} ?>
			<li><a href="javascript:void(0);" <?php if( $brand_id > 0 ){?> onclick="brandMediaForm(<?php echo $brand_id ?>);" <?php }?>><?php echo Labels::getLabel('LBL_Media',$adminLangId); ?></a></li>
		</ul>
