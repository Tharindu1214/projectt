...
foreach ($arr_listing as $sn => $row) {
    ...
    $tr->setAttribute("id", $row['city_id']);

    foreach ($arr_flds as $key => $val) {
        $td = $tr->appendElement('td');
        switch ($key) {
            case 'select_all':
                $td->appendElement('plaintext', array(), '<label class="checkbox"><input class="selectItem--js" type="checkbox" name="city_ids[]" value='.$row['city_id'].'><i class="input-helper"></i></label>', true);
                break;
            case 'listserial':
            ...
            case 'action':
                ...
                if ($canEdit) {
                    ...
                    $innerLiEdit->appendElement('a', array('href'=>'javascript:void(0)', 'class'=>'button small green', 'title'=>Labels::getLabel('LBL_Edit', $adminLangId),"onclick"=>"editStateFormNew(".$row['city_id'].")"), Labels::getLabel('LBL_Edit', $adminLangId), true);
                }
                break;
            case 'city_active':
                $active = "";
